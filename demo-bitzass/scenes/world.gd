...
var active_joycon : JoyCon:
	set(jc):
		active_joycon = jc
		print("active joycon: ", active_joycon)
...
func joycon_pressed_y(joycon : JoyCon):
	if active_menu:
		...
		if not active_joycon or active_joycon == joycon:
...
func joycon_pressed_r(joycon : JoyCon):
	if active_menu:
		return
	if joycon != active_joycon:
		return
	if active_joycon.b_pressed:
		active_joycon = null
		print("null jc")
...
func add_joycon(joycon : JoyCon):
	...
	joycon.r_just_pressed.connect(joycon_pressed_r.bind(joycon))
