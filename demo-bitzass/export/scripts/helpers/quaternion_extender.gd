...
static func my_quat_interpolate(q1 : Quaternion, ax1 : Vector3, v1 : float, q2 : Quaternion, ax2 : Vector3, v2 : float, t : float, dur : float, ease_points : PackedFloat32Array) -> Quaternion:
	ax1 = ax1.normalized()
	ax2 = ax2.normalized()
	var angle_1 := v1 * dur* t
...
static func get_auto_velocity(prev_q : Quaternion, prev_ax : Vector3, prev_v : float, next_q : Quaternion, next_ax : Vector3, next_v : float, dur : float, next_points : PackedFloat32Array) -> float:
	var t_start := 0.5
	var small_amount := 0.001
	var t_plus := t_start + small_amount
	var q1 := my_quat_interpolate(prev_q, prev_ax, prev_v, next_q, next_ax, next_v, t_start, dur, next_points)
	var q2 := my_quat_interpolate(prev_q, prev_ax, prev_v, next_q, next_ax, next_v, t_plus, dur, next_points)
